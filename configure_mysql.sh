...
mysql --batch --silent -e CREATE DATABASE wordpress DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci >>/script_output.txt

echo "####	Grant permissions to Wordpress DB to new wordpress user account">>/script_output.txt
mysql --batch --silent -e GRANT ALL ON wordpress.* TO 'wordpressuser'@'%' IDENTIFIED BY '$CS_SQL_PASSWD' >>/script_output.txt